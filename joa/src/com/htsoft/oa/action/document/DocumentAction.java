...
public class DocumentAction extends BaseAction{
	...
	public String shareList(){
		...
		List<Document> list=documentService.findByIsShared(document,from,to,appUser.getUserId(), arrayList, dep.getDepId(),pb);
	...
	public String list(){
		...
		List<Document> list= documentService.findByPersonal(ContextUtil.getCurrentUserId(), document, from, to, path, pb);
		Type type=new TypeToken<List<Document>>(){}.getType();
		StringBuffer buff = new StringBuffer("{success:true,'totalCounts':")
		.append(pb.getTotalItems()).append(",result:");		
		Gson gson=new Gson();
...
//				}
	...
	public String save(){
		...
		if(document.getDocId()==null){
			document.setAppUser(ContextUtil.getCurrentUser());
