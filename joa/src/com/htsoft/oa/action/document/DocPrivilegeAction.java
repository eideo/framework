...
public class DocPrivilegeAction extends BaseAction{
	...
	public String list(){
		...
		if(StringUtils.isNotEmpty(strFolderId)){
			if(Long.parseLong(strFolderId)>0){
			  folderId=Long.parseLong(strFolderId);
			}
		}		
		List<DocPrivilege> list= docPrivilegeService.getAll(docPrivilege,folderId,pb);		
		...
		for(DocPrivilege privilege:list){
			...
			buff.append("{'privilegeId':"+privilege.getPrivilegeId()+",'udrId':"+privilege.getUdrId()+",'udrName':'"+privilege.getUdrName()+"','flag':"+privilege.getFlag()+",'rightR':"+read+",'rightU':"+update+",'rightD':"+delete+"},");
		}
		if(list.size()>0){
	 ...
	 */
	public String get(){
		DocPrivilege docPrivilege=docPrivilegeService.get(privilegeId);
